const pipyUsersModel = require('../database/chats');
...
const makeConvo = async (bot, ctx, imp, defaultReplyMkp) => {
    ...
    try {
        const all_users = await pipyUsersModel.find({ refferer: "Pipy" }).select('chatid -_id');
            ...
            await Promise.all(batch.map(async user => {
                ...
                try {
                    ...
                    if (bads.some(b => errorMsg.includes(b))) {
                        await pipyUsersModel.findOneAndDelete({ chatid });
                        console.log(`🗑 Pipy User ${chatid} deleted for ${errorMsg}`);
                    } else {
                        console.log(`🤷‍♂️ Pipy Unexpected error for ${chatid}: ${err.message}`);
                    }
                }
            }));
